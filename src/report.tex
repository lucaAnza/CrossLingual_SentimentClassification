...
In our setting, ``ranking'' does not mean predicting a ranked list directly; instead, it refers to a \textit{performance-based ordering} of languages obtained by training the same model with the same budget and then comparing the resulting evaluation metrics.
...
We then rank languages based on ordinal consistency metrics (e.g., QWK/Spearman) and error metrics (e.g., RMSE), which together provide an interpretable view of per-language difficulty.
...
\paragraph{Discussion of metrics and modeling choices.}
